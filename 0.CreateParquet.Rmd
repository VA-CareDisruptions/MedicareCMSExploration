...
csv_stream_inp_k <- open_dataset("T:/inp/inp1/basek2016_2021", format = "csv", schema = sch_inp_basek) %>%
  dplyr::select(DSYSRTKY,CLAIMNO,THRU_DT, starts_with('ICD'),starts_with('PRCDR'), TOT_CHRG,ADMSN_DT,TYPE_ADM,SRC_ADMS,PCCHGAMT,
                DOB_DT, RACE_CD,CNTY_CD,STATE_CD,DSCHRGDT, UTIL_DAY, COIN_DAY,PRNCPAL_DGNS_CD)
...
write_dataset(csv_stream_inp_k, "T:/parquet/inp_k/", format = "parquet", 
...
csv_stream_outp <- open_dataset("T:/out/basek2016_2021", format = "csv", schema = sch_outp_basek)  %>%
  dplyr::select(DSYSRTKY,CLAIMNO,THRU_DT, starts_with('ICD'),starts_with('PRCDR'), TOT_CHRG,PCCHGAMT,
                DOB_DT, RACE_CD,CNTY_CD,STATE_CD)
