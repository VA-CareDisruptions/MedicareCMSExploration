...
```{r} 
pq_stream_inp_k <- open_dataset("T:/parquet/inp_k/", format = "parquet" , schema=sch_inp_basek) 

pq_stream_inp_j <- open_dataset("T:/parquet/inp_j/", format = "parquet", schema=sch_inp_basej)

#pq_stream_out <- open_dataset("T:/parquet/outp/", format = "parquet", schema=sch_outp_basek)
```
...
LOS_trends_src <- pq_stream_inp_k %>%
  dplyr::select(UTIL_DAY,COIN_DAY,ADMSN_DT,SRC_ADMS,STUS_CD) %>%
  mutate(LOS = UTIL_DAY + COIN_DAY) %>%
  ...
  ungroup() %>%
  group_by(monthdate,SRC_ADMS,STUS_CD) %>%
  summarize(LOS = mean(LOS), N=sum(N)) #this should be weigted mean, not straight mean 
